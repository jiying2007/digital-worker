...
- [嵌入式系统专家团目录](expert-groups/embedded-system/README.md)：已落地 Governance Skeleton、1+7 Core Experts、P0 Skills、共享 Engineering Handoff Contract、核心 Schema 与自动结构校验 CI。
...
- 嵌入式系统专家团架构状态为 `architecture-frozen`；当前实现状态已推进到 `p0-skills-and-engineering-handoff-defined`。这表示专家团已有结构化接单和工程交接基线，但尚未通过 Golden Cases 与真实项目 Pilot，因此不是 Production Ready。
- 嵌入式系统专家团的 Agent、Skill、Workflow、Schema 等正式资产以本仓为 SSOT；其他外部仓库仅作参考或未来可选集成，不构成当前运行前置依赖。

完成身份映射、知识权限、飞书工作项接口和试点产品范围确认后，再由决策人评审研发流程 V2/ADR-001 是否转为 `accepted`。嵌入式专家团下一阶段按 Cross-Team Contract → Golden Cases/Regression → Pilot 顺序推进。
