...
class EdgeTargetRuntimeTests(unittest.TestCase):
    def test_target_runtime_has_no_legacy_identity(self):
        for rel in ["runtime/task-modes.yaml", "runtime/workflow.yaml"]:
            text = (EDGE / rel).read_text(encoding="utf-8")
            for legacy in LEGACY_IDS:
                self.assertNotIn(legacy, text, f"{legacy} leaked into {rel}")

    def test_target_runtime_covers_all_task_types(self):
        routing = yaml.safe_load((EDGE / "runtime/task-modes.yaml").read_text(encoding="utf-8"))["routing"]
        target = yaml.safe_load((EDGE / "routing-shadow.yaml").read_text(encoding="utf-8"))["routing"]
        self.assertEqual(set(routing), set(target))
        self.assertEqual(len(routing), 14)

    def test_target_pilot_and_schemas_are_present(self):
    ...
    def test_target_cli_can_initialize_and_scaffold_without_legacy_data_path(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            subprocess.run([
                sys.executable, str(ROOT / "scripts/edge_pilot.py"), "init",
                "--run-id", "EDGE-TARGET-CI-001",
                "--track", "feature",
                "--source-type", "synthetic",
                "--task-type", "feature_development",
                "--workflow-mode", "short_chain",
                "--human-owner", "ci",
                "--task-brief", str(ROOT / "tests/fixtures/task-brief.valid.json"),
                "--repo-root", "firmware/main",
                "--base-commit", "0123456789abcdef0123456789abcdef01234567",
                "--output-root", str(root),
            ], check=True)
            run_dir = root / "EDGE-TARGET-CI-001"
            subprocess.run([sys.executable, str(ROOT / "scripts/edge_pilot_scaffold.py"), str(run_dir)], check=True)
