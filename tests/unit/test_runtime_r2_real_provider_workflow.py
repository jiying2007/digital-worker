...
class RuntimeR2RealProviderWorkflowTests(unittest.TestCase):
    ...
    def test_is_manual_only_and_requires_explicit_human_confirmation(self) -> None:
        ...
        self.assertIn("BLOCKED_MISSING_OPENAI_API_KEY", self.text)
        self.assertIn("BLOCKED_MISSING_ANTHROPIC_API_KEY", self.text)
    ...
    def test_actions_are_full_sha_pinned(self) -> None:
        ...
        self.assertIn("openai/codex-action@5c3f4ccdb2b8790f73d6b21751ac00e602aa0c02", self.text)
        self.assertIn("anthropics/claude-code-action/base-action@3b8197d3d486006dd4af54613517f21ac6ac625e", self.text)
    ...
    def test_real_runtimes_consume_isolated_bound_source_sets(self) -> None:
        ...
        self.assertIn('cp -a "$RUNNER_TEMP/claude-materialized/distribution/." "$RUNNER_TEMP/claude-home/.claude/"', self.text)
    ...
    def test_receipt_collection_is_non_terminal_and_cannot_write_root_pass(self) -> None:
        self.assertIn("provider-executions-collected-pending-digital-worker-verification-review", self.text)
        ...
        self.assertNotRegex(self.text, r"['\"]verification_pass_claimed['\"]\s*:\s*[Tt]rue")
