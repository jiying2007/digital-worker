...
def main() -> None:
    ...
    require("R1_BINDINGS_READY" in runtime_eval.get("validation", ""), "llm_agent must project both R1 source-set-bound runtimes")
    require("REAL_R2_EVIDENCE_PENDING" in runtime_eval.get("validation", ""), "llm_agent real R2 evidence blocker must remain explicit")
    ...
    claude = lock["runtime_bindings"].get("claude", {})
    ...
    require("L2_BOOTSTRAP" in claude.get("validation", ""), "Claude L2 bootstrap capability must be explicitly promoted")
    require("RECEIPT_V2" in claude.get("validation", ""), "Claude receipt v2 capability must be explicitly promoted")
    ...
    require(
        cap_claude["identity_ref"] == "config/integrations/cross-repo-lock.json#/runtime_bindings/claude",
        "Claude capability projection must reference the authoritative cross-repo lock identity",
    )
    require(cap_claude["capabilities"]["thin_session_bootstrap_l0_l1_l2"] == "native", "Claude thin bootstrap capability missing")
    require(cap_claude["capabilities"]["source_set_materialization"] == "native", "Claude source-set materialization capability missing")
    require(cap_claude["capabilities"]["execution_receipt_contract"] == "source-set-v2-native", "Claude capability projection must require receipt v2")
    require(cap_claude["capabilities"]["real_provider_execution_receipt"] == "pending-external-execution", "Claude real execution receipt must remain pending")
    require(cap_claude["capabilities"]["r2_real_provider_substitution"] == "pending-external-evidence", "Claude R2 evidence must remain pending")
    ...
    print(
        ...
        "canonical domain Skills, ADK reusable-asset boundary, dual R1 source-set-bound Runtime bindings, "
