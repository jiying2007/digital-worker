#!/usr/bin/env python3
"""Fail-closed synchronization checks for embedded human views and digital positions."""
...
ORG_DIR = CORE / "01-数字组织与岗位"
OVERVIEW = ORG_DIR / "01 数字员工组织与运行总览.md"
POSITIONS = ORG_DIR / "03 数字岗位与能力模型.md"
HUMAN = ORG_DIR / "human-view.yaml"
...
def main() -> None:
    require(OVERVIEW.is_file(), f"missing organization overview: {OVERVIEW.relative_to(ROOT)}")
    ...
    require(CORE_README.is_file(), "core README must be the single human entry")
    ...
    require(not any(p.exists() for p in forbidden), "superseded IA, workbook or parallel Position SSOT residue must stay absent")
    ...
    require(human["schema_version"] == 1, "unexpected human-view schema_version")
    require(human.get("overview_view_version") == "1.1.0", "unexpected human-view version")
    require(human.get("status") == "operational-overview", "human-view must remain operational-overview")
    require("workbook" not in human_text.lower(), "human-view must not retain workbook-era naming")
    require("唯一第一入口" in core_readme, "root README must be the single human entry")

    for marker in ["1 名主理人 + 7 个专业角色", "E2 Engineering Closed Loop", "Production Ready", "人类总览视图", "第二个 SSOT"]:
        require(contains_compact(text, marker), f"organization overview baseline marker missing: {marker}")
    for marker in ["23", "14", "7", "K / M / 0 / T / E / V / R / C", "A0-A7"]:
        require(marker in text, f"organization overview baseline marker missing: {marker}")
    ...
    require(set(role_ids) == set(human["roles"]), "human-view role set drift")
    ...
    for item in skills:
        require(item["id"] in text, f"organization overview skill missing: {item['id']}")
    ...
    require(set(routing) == set(human["task_labels"]) == set(human["task_guidance"]), "human-view task set drift")
    for task_type, cfg in routing.items():
        ...
        for expert_id in cfg["primary_experts"]:
            require(expert_id in human["roles"], f"routing expert missing from human role model: {task_type} -> {expert_id}")
    ...
    require(
        contains_compact(position_text, "禁止跨层推导") or contains_compact(position_text, "不允许跨层推导"),
    ...
    for entry in [
        ...
        require(entry.split("/", 1)[0] in core_readme, f"core README missing IA section: {entry}")
    require("positions.yaml" in core_readme and "不是新的 Position SSOT" in core_readme, "core README must forbid parallel Position SSOT")

    print("embedded overview PASS: single-entry IA + 6 modules / 8 positions synchronized with 1+7 Agents / 23 skills / 14 tasks / 7 modes / 8 gates / A0-A7 / 7 verification layers")
