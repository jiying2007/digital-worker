#!/usr/bin/env python3
"""Validate Edge Foundation Material Manifest semantics and terminal readiness."""
...
SCHEMA = ROOT / "domains" / "edge-foundation" / "schemas" / "material-manifest.schema.json"
...
def validate_manifest(manifest: dict, track: str, require_terminal_ready: bool = False) -> dict:
    ...
    return {"run_id": manifest["run_id"], "track": track, "readiness": readiness, "blockers": blockers, "terminal_acceptable": readiness != "BLOCKED"}
...
def main() -> None:
    ...
    if args.output:
        ...
        print(rendered, end="")
    print(f"material manifest validation PASS: run={result['run_id']} track={result['track']} readiness={result['readiness']}")
